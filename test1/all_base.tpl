...
proxies: ~
  # 链式代理节点（Chain Proxy Nodes）- 从 config.yaml 读取配置
  # 注意：dialer-proxy 的值必须与订阅源中的节点名称完全匹配
  # 配置来源：pref.ini 的 [template] 部分，可通过脚本从 config.yaml 自动更新
  {% if exists("local.chain_proxy.enable") and local.chain_proxy.enable == "true" %}
  - name: "{{ local.chain_proxy.name | default('台湾Akile') }}"
    dialer-proxy: "{{ local.chain_proxy.dialer_proxy | default('AutoTW 🇨🇳') }}"
    type: {{ local.chain_proxy.type | default('ss') }}
    server: "{{ local.chain_proxy.server | default('akilehinetnat.645781.xyz') }}"
    port: {{ local.chain_proxy.port | default(10490) }}
    cipher: "{{ local.chain_proxy.cipher | default('aes-128-gcm') }}"
    password: "{{ local.chain_proxy.password | default('db756bc3-09ef-4550-82e0-d3c4395e8348') }}"
    udp: {{ local.chain_proxy.udp | default(true) }}
  {% endif %}
proxy-groups: ~
rules: ~
{% else %}
